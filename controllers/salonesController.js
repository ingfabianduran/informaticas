...
module.exports = {
    ...
    viewDahsboard: function(req, res, next)
    {
        ...
        {
            knex.select("codigo", "nombreCompleto", "privilegio").from("usuario").orderBy("nombreCompleto", "ASC").then(function(usuarios)
            {
                knex.select("codigo", "nombre", "cantidadCargadores").from("salon").orderBy("nombre", "ASC").then(function(salones){

                    var date = "";

                    if (moment().format("MM") < 7) date = "Reporte por Mes / " + moment().format("YYYY") + " - 1";
                    else date = "Reporte por Mes / " + moment().format("YYYY") + " - 2";

                    const meses = {data: [{mes: "Enero", value: "01"}, {mes: "Febrero", value: "02"}, {mes: "Marzo", value: "03"}, {mes: "Abril", value: "04"}, {mes: "Mayo", value: "05"}, {mes: "Junio", value: "06"}, {mes: "Julio", value: "07"}, {mes: "Agosto", value: "08"}, {mes: "Septiembre", value: "09"}, {mes: "Octubre", value: "10"}, {mes: "Noviembre", value: "11"}, {mes: "Diciembre", value: "12"},]};
                    const privilegios = {data: [{value: "estandar", text: "Estandar"}, {value: "administrador", text: "Administrador"}]};
                    const tiposAula = {data: [{value: "fijo", text: "Fijo"}, {value: "movil", text: "Movil"}]};
                    
                    res.render("salones", {data: {usuarios: usuarios, salones: salones, meses: meses, privilegios: privilegios, tiposAula: tiposAula, date: date, message: req.flash("error"), usuario: req.session.nombreCompleto, privilegio: req.session.privilegio, avatar: req.session.avatar}});
    ...
    viewReportSalones: function(req, res)
    {
        ...
        const sql = "SELECT SUM((DATE_FORMAT(hFinal.hora, '%k') - DATE_FORMAT(hInicio.hora, '%k'))) AS total, DATE_FORMAT(fecha, '%M') AS mes FROM clase JOIN horario AS hInicio ON clase.horaInicio = hInicio.codigo JOIN horario AS hFinal ON clase.horaFinal = hFinal.codigo WHERE fecha BETWEEN '" + dataDate.fechaInicial + "' AND '" + dataDate.fechaFinal + "' GROUP BY MONTH(fecha) ORDER BY MONTH(fecha)";

        knex.raw(sql).then(function(report)
        {
    ...
    getInfoPresCost: function(req, res) 
    {
        if (req.session.status && req.session.nombre && req.session.nombreCompleto && req.session.privilegio)
        {
            ...
            if (result.error == null)
            {
                ...
                {
                    const dataDate = {
                        fechaInicial: moment().format("YYYY") + "/" + value.value + "/" + "01",
                        fechaFinal: moment().format("YYYY") + "/" + value.value + "/" + "31",
                    };

                    const sql = "SELECT programaacademico.nombre, SUM(DATE_FORMAT(hFinal.hora, '%k') - DATE_FORMAT(hInicio.hora, '%k')) AS total FROM clase JOIN horario AS hInicio ON clase.horaInicio = hInicio.codigo JOIN horario AS hFinal ON clase.horaFinal = hFinal.codigo JOIN programaacademico ON clase.programaAcademico = programaAcademico.codigo WHERE fecha BETWEEN '" + dataDate.fechaInicial + "' AND '" + dataDate.fechaFinal + "' GROUP BY programaAcademico.nombre";
                    ...
                    knex.raw(sql).then(function(report)
                    {
                        ...
                        report[0].forEach(element => {
                            ...
                            {
                                if (index == 1) book.cell(fila, index).string(element.nombre).style(stylesBody);
                                if (index == 2) book.cell(fila, index).number(element.total).style(stylesBody);
                            }
                            fila ++;
                        });
        ...
        if (req.session.status && req.session.nombre && req.session.nombreCompleto && req.session.privilegio)
        {
            ...
            if (validate.error == null)
            {
                knex("salon").insert({
                    ...
                    res.send({data: {status: true, message: '<h3 class="font-italic">Salón creado correctamente!!!</h3>'}});
                }).catch(function(err){
                    res.send({data: {status: false, message: "Ops!!! algo raro paso aca"}});
                });
            }
    ...
    moreInfoSalon: function(req, res)
    {
        if (req.session.status && req.session.nombre && req.session.nombreCompleto && req.session.privilegio)
        {
            ...
            if(result.error == null)
            {
                knex.select("nombre", "cantidadCargadores", "observaciones", "tipo").from("salon").where("codigo", data.codigo).then(function(salon){
    ...
    updateInfoSalon: function(req, res)
    {
        if (req.session.status && req.session.nombre && req.session.nombreCompleto && req.session.privilegio)
        {
            ...
            if (validate.error == null)
            {
                knex("salon").where("codigo", data.codigo).update({
                    nombre: data.nombre,
                    cantidadCargadores: data.cargadores,
                    observaciones: data.observaciones,
                    tipo: data.tipo}).then(function(result){
                        res.send({data: {status: true, message: "Salón actualizado correctamente!!!"}});
                    }).catch(function(err){
                        res.send({data: {status: false, message: "Ops!!! Algo raro paso por aquí"}});
                });
            }
            else
            {
